...
def fig_F0(data, out):
    ...
    ax.set_title("F0 · Convergencia al estado estacionario")
...
def fig_F1_F2_F3(data, out):
    ...
    ax.set_title("F1 · Cooperación interna media (modelo canónico)")
    for i in range(nrow):
        for j in range(ncol):
            v = piv.values[i, j]
    ...
    ax.set_title("F2 · La presión de imitación ($\\delta_\\pi$) hunde la cooperación")
    ax.set_ylim(0, None); ax.legend()
    ...
    ax.set_ylabel("proporción de cooperadores")
    ax.set_title("F3 · La adaptación social ($\\eta_s$) y su óptimo intermedio")
    ax.set_ylim(0, None); ax.legend()
...
def fig_F4(data, out):
    ...
    ax.set_title("F4 · Una meta-red sparse hace variar el clima (y se propaga)")
    ax.invert_xaxis()
...
def fig_F5(data, out):
    ...
    ax.set_title("F5 · El acoplamiento no mueve la cooperación por ningún canal")
